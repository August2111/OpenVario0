...
myprocess = subprocess.Popen(['docker', 'image_name', 'ls', '-q', image_name], env = my_env, cwd = cwd, stdout = out, shell = False)
myprocess.wait()
out.close()
...
set_target_dir = ''
build_cmd  = target_dir + 'build-image.py'
...
myprocess = subprocess.Popen(['docker', 'run', '--rm', '--mount', 'type=bind,source='+ cwd + ',target=' + target_dir, '-it', image_name
       , set_target_dir, build_cmd], env = my_env, cwd = cwd, shell = False)
myprocess.wait()
